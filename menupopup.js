	...
	if (options.hasGoTo) {
		var gotos = [],
			...
			// winmtr
                        winmtr = {
                                label: t('Winmtr'),
                                url: new Curl('portal.php?app=winmtr&hostid=' + options.hostid).getUrl()
                        },  
        	        // winbox
                	winbox = {
                        	label: t('Winbox'),
                	...
                	putty = {
                        	label: t('Putty'),
                        	url: new Curl('portal.php?app=putty&hostid=' + options.hostid).getUrl()
                	},
                	//web
                	web = {
                        	label: t('Web'),
                        	url: new Curl('portal.php?app=web&hostid=' + options.hostid).getUrl()
                	},
			//rdp
                        rdp = {
                                label: t('rdp'),
                                url: new Curl('portal.php?app=rdp&hostid=' + options.hostid).getUrl()
                        },
                	// mapa interativo
                	imap = {
                        	label: t('Mpa Interativo'),
                        	url: new Curl('imap.php?hostid2=' + options.hostid).getUrl()
                	};
		...
		gotos[gotos.length] = winmtr;		
		gotos[gotos.length] = winbox;
		gotos[gotos.length] = putty;
		gotos[gotos.length] = web;
		gotos[gotos.length] = rdp;
		gotos[gotos.length] = imap;
	...
	if (typeof options.gotos !== 'undefined') {
		...
		if (typeof options.gotos.screens !== 'undefined') {
			...
			gotos[gotos.length] = screens;
                                                ...
                                                // winmtr
                                                var url = new Curl('portal.php?app=winmtr&hostid=' + options.hostid);
                                                gotos[gotos.length] = {
                                                                label: t('Winmtr'),
                                                                url: url.getUrl()
						}
						// winbox
						var url = new Curl('portal.php?app=winbox&hostid=' + options.hostid);
						gotos[gotos.length] = {
								label: t('Winbox'),
								url: url.getUrl()
						}
						// putty
						var url = new Curl('portal.php?app=putty&hostid=' + options.hostid);
						gotos[gotos.length] = {
								label: t('Putty'),
								url: url.getUrl()
						}
						// web
						var url = new Curl('portal.php?app=web&hostid=' + options.hostid);
						gotos[gotos.length] = {
								label: t('Web'),
								url: url.getUrl()
						}
						// mapa interativo
						var url = new Curl('imap.php?hostid2=' + options.hostid);
                                                gotos[gotos.length] = {
                                                                label: t('Mpa Interativo'),
                                                                url: url.getUrl()
                                                }
